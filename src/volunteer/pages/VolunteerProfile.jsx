...
import { updateVolunteerAPI } from "../../sevices/Allapi";
...
export default function VolunteerProfile() {
  ...
  const { setVolunteerProfileUpdated } = useContext(
    VolunteerProfileUpdateContext
  );
  ...
  const updateProfile = async () => {
    ...
    if (username && phone && password) {
      ...
      if (preview) {
        ...
        const result = await updateVolunteerAPI(reqBody, reqHeader);

        if (result.status == 200) {
          toast.success("profile updated successfully");
          sessionStorage.setItem("users", JSON.stringify(result.data));
          setVolunteerProfileUpdated(result.data);
        }
      } else {
        const result = await updateVolunteerAPI(profile, reqHeader);
